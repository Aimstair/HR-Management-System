...
const makeEmployee = (
  ...
  lastName: fullName.split(' ').slice(2).join(' ') || fullName.split(' ')[1] || '',
  id,
  fullName,
  jobTitle,
  profilePicture: `https://picsum.photos/seed/${id.toLowerCase()}/120/120`,
  birthDate: '1990-01-01',
  phoneNumber: '+63 917 555 0000',
  presentAddress: 'Metro Manila',
  presentAddressZipCode: '1000',
  permanentAddress: 'Metro Manila',
  permanentAddressZipCode: '1000',
  username: `${fullName.toLowerCase().replace(/[^a-z]+/g, '.').replace(/(^\.|\.$)/g, '')}`,
  temporaryPassword: 'Welcome123!',
  idNumber: `ID-${id.slice(-4)}`,
  position: jobTitle,
  ...
  schoolName: role === 'Faculty' || role === 'Chairman' || role === 'Dean' ? 'Teaching School' : 'N/A',
  departmentName: jobTitle.split(',')[0] || 'General',
  privileges: {
    noLate: false,
    noOvertime: false,
